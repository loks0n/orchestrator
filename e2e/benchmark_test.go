...
func TestConcurrentJobsWithCallbacks(t *testing.T) {
	...
	callbacks.Range(func(_, value any) bool {
		totalCallbacks.Add(value.(*atomic.Int64).Load())
		return true
	})
	...
	if startCallbacks < int64(float64(created.Load())*0.8) {
		t.Errorf("Expected at least 80%% start callbacks, got %d/%d", startCallbacks, created.Load())
	}
	if exitCallbacks < int64(float64(created.Load())*0.8) {
		t.Errorf("Expected at least 80%% exit callbacks, got %d/%d", exitCallbacks, created.Load())
	}
}
...
// TestCircuitBreakerUnderFailure tests circuit breaker behavior with failing endpoint.
func TestCircuitBreakerUnderFailure(t *testing.T) {
	...
	for i := 0; i < numEvents; i++ {
		event := &dispatcher.Event{
			Payload:     newTestEvent(fmt.Sprintf("breaker-%d", i)),
			Destination: callbackServer.URL,
		}
		d.Dispatch(event)
	}
	...
	// Circuit breaker cooldown is 30s, so we need to wait longer than that
	testutil.WaitFor(t, func() bool {
		stats := d.Stats()
		return stats.Delivered > 0 && (stats.Delivered+stats.Failed+stats.Dropped) >= int64(numEvents*0.5)
	}, testutil.WithTimeout(45*time.Second))

	stats := d.Stats()
	elapsed := time.Since(start)

	t.Logf("=== Circuit Breaker Test ===")
	...
	successRate := float64(stats.Delivered) / float64(numEvents) * 100
	t.Logf("Success rate:     %.1f%%", successRate)

	// With requeuing, events should eventually be delivered after circuit recovers
