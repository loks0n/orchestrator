...
// TestSidecar_FullFlow tests the sidecar's I/O handling.
// Note: start/exit events are now handled by the Docker orchestrator, not the sidecar.
// The sidecar only handles input downloads and output processing.
func TestSidecar_FullFlow(t *testing.T) {
	...
	cfg := &sidecar.Config{
		...
		CallbackEvents:   "orchestrator.job.output",
		TimeoutSeconds:   60,
		SharedVolumePath: sharedDir,
		OutputsJSON:      `[{"id":"result","type":"event","path":"output.txt"}]`,
	}
	...
	// Verify we received the output event
	hasOutputEvent := false
	for _, event := range receivedEvents {
		if event["type"] == "orchestrator.job.output" {
			hasOutputEvent = true
			break
		}
	}
	...
	if !hasOutputEvent {
		t.Error("Missing expected output event")
	}
...
func TestSidecar_InputDownload(t *testing.T) {
	...
	inputsJSON := fmt.Sprintf(`[{"id":"input-1","type":"download","path":"input.txt","url":"%s"}]`, inputServer.URL)
	cfg := &sidecar.Config{
		JobID:            jobID,
		InputsJSON:       inputsJSON,
		CallbackURL:      callbackServer.URL,
		CallbackEvents:   "orchestrator.job.input",
		TimeoutSeconds:   60,
		SharedVolumePath: sharedDir,
	}
	...
	hasInputEvent := false
	for _, event := range receivedEvents {
		if event["type"] == "orchestrator.job.input" {
			hasInputEvent = true
			if data, ok := event["data"].(map[string]any); ok {
				if data["status"] != "success" {
					t.Errorf("Input event status: got %v, want 'success'", data["status"])
				}
			}
			break
		}
	...
	if !hasInputEvent {
		t.Error("No input event received")
	}
}
...
func TestSidecar_OutputUpload(t *testing.T) {
	...
	outputsJSON := fmt.Sprintf(`[{"id":"result","type":"upload","path":"result.txt","url":"%s"}]`, uploadServer.URL)
	cfg := &sidecar.Config{
		JobID:            jobID,
		CallbackURL:      callbackServer.URL,
		CallbackEvents:   "orchestrator.job.output",
	...
	for _, event := range receivedEvents {
		if event["type"] == "orchestrator.job.output" {
			...
			if data, ok := event["data"].(map[string]any); ok {
				if data["status"] != "success" {
					t.Errorf("Output event status: got %v, want 'success'", data["status"])
				}
			}
			break
		}
