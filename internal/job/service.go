...
func (s *Service) validate(req *Request) error {
	...
	if len(req.Inputs) > maxInputs {
		return apperrors.Validation("inputs", fmt.Sprintf("inputs exceed maximum of %d", maxInputs))
	}
	for i, input := range req.Inputs {
		field := fmt.Sprintf("inputs[%d]", i)
		if input.ID == "" {
			return apperrors.Validation(field+".id", fmt.Sprintf("input[%d]: ID is required", i))
		}
		if input.Type == "" {
			return apperrors.Validation(field+".type", fmt.Sprintf("input[%d]: type is required", i))
		}
		if input.Type != "download" && input.Type != "file" {
			return apperrors.Validation(field+".type", fmt.Sprintf("input[%d]: invalid type %q (must be download or file)", i, input.Type))
		}
		if input.Path == "" {
			return apperrors.Validation(field+".path", fmt.Sprintf("input[%d]: path is required", i))
		}
		if err := validatePath(input.Path); err != nil {
			return apperrors.Validation(field+".path", fmt.Sprintf("input[%d]: invalid path: %v", i, err))
		}
		if input.Type == "download" {
			if input.URL == "" {
				return apperrors.Validation(field+".url", fmt.Sprintf("input[%d]: URL is required for download type", i))
			}
			if err := validateURL(input.URL); err != nil {
				return apperrors.Validation(field+".url", fmt.Sprintf("input[%d]: invalid URL: %v", i, err))
			}
		}
		if input.Type == "file" && input.Content == "" {
			return apperrors.Validation(field+".content", fmt.Sprintf("input[%d]: content is required for file type", i))
		}
	}

	// Validate outputs
	if len(req.Outputs) > maxOutputs {
		return apperrors.Validation("outputs", fmt.Sprintf("outputs exceed maximum of %d", maxOutputs))
	}
	for i, output := range req.Outputs {
		field := fmt.Sprintf("outputs[%d]", i)
		if output.ID == "" {
			return apperrors.Validation(field+".id", fmt.Sprintf("output[%d]: ID is required", i))
		}
		if output.Type == "" {
			return apperrors.Validation(field+".type", fmt.Sprintf("output[%d]: type is required", i))
		}
		if output.Type != "upload" && output.Type != "event" {
			return apperrors.Validation(field+".type", fmt.Sprintf("output[%d]: invalid type %q (must be upload or event)", i, output.Type))
		}
		if output.Path == "" {
			return apperrors.Validation(field+".path", fmt.Sprintf("output[%d]: path is required", i))
		}
		if err := validatePath(output.Path); err != nil {
			return apperrors.Validation(field+".path", fmt.Sprintf("output[%d]: invalid path: %v", i, err))
		}
		if output.Type == "upload" {
			if output.URL == "" {
				return apperrors.Validation(field+".url", fmt.Sprintf("output[%d]: URL is required for upload type", i))
			}
			if err := validateURL(output.URL); err != nil {
				return apperrors.Validation(field+".url", fmt.Sprintf("output[%d]: invalid URL: %v", i, err))
			}
		}
	}
