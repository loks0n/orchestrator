...
Async job execution via Docker containers with webhook callbacks. Jobs share the default bridge network and use a dedicated volume for I/O between the job container and sidecar.
...
flowchart TB
    ...
    Sidecar -->|signed input/output events| Service
...
### Sidecar for I/O, Orchestrator for Lifecycle

The sidecar (`cmd/job-sidecar`) handles file I/O (downloads, uploads) while the orchestrator streams logs and detects exit via Docker API.

**Why?** If the service restarts mid-job, the orchestrator can reconnect to Docker and resume log streaming. The sidecar only handles I/O which completes before/after the job runs.
...
See: `internal/docker/docker.go` -> `reconcile()`, `watchUntilExit()`
...
By default, sidecar callbacks (input/output events) are routed through the orchestrator.
...
See: `internal/api/handler.go` -> `ProxyEvent()`, `internal/docker/docker.go` -> `createSidecarContainer()`
...
See: `internal/docker/state.go` -> `reserve()`, `commit()`, `release()`
...
See: `internal/docker/docker.go` -> `reconcile()`
...
See: `internal/docker/docker.go` -> `runMaintenance()`, `cleanupExpiredJobs()`
...
See: `internal/sidecar/output.go` -> `upload()`, `uploadFile()`

## Callback Events

CloudEvents 1.0 format with optional HMAC-SHA256 signing. Events are sent for: `start`, `log`, `exit`, `input`, `output`.
...
See: `cmd/jobs-service/main.go`, `internal/docker/docker.go` -> `Close()`
